  ...
  <div class="page">
    ...
    <header class="header">
      <div>
        ...
        <p>Questions and answers from arts practice</p>
      </div>
      <div class="header-actions">
        <span class="lang-switch"><a href="faq.php">Deutsch</a></span>
    ...
    <main id="main-content">
      ...
      <div class="faq-hero">
        ...
        <h1>artbackstage answers your questions</h1>
        <p>Here you'll find answers to frequently asked questions about finance, law, and social security in arts practice.</p>
      </div>

      <!-- FAQ card -->
<?php if ($dbError): ?>
      <div class="faq-empty">
        <p>FAQ entries could not be loaded right now. Please try again later.</p>
      </div>
<?php elseif ($faqEntry === null || $faqEntry === false): ?>
      <div class="faq-empty">
        <p>No questions and answers yet. Check back soon!</p>
      </div>
<?php else:
...
?>
      <div class="faq-card" id="faq-card">
        <div class="faq-card-meta">
          ...
          <?php endif; ?>
          <?php if ($faqTotal > 1): ?>
          <span class="faq-counter"><?= $faqTotal ?> questions in total</span>
          <?php endif; ?>
        </div>
        ...
        <div class="faq-vote-row" id="vote-row">
          <span class="faq-vote-label">Was this answer helpful?</span>
          <button class="vote-btn" id="btn-pos" onclick="castVote(<?= $faqId ?>, 'positive')" aria-label="Yes, helpful">
            <span aria-hidden="true">+</span> Yes
            <span class="vote-count" id="count-pos">(<?= $votesPos ?>)</span>
          </button>
          <button class="vote-btn" id="btn-neg" onclick="castVote(<?= $faqId ?>, 'negative')" aria-label="Not helpful">
      ...
      <div class="faq-next-row">
        <a class="btn btn-outline" href="faq_en.php">Show another question ↺</a>
      </div>
      <?php endif; ?>
...
<?php endif; ?>
      ...
      <div class="faq-contact">
        <h2>Your question isn't listed?</h2>
        <p>Send us your question – we promise a short answer within the next few days.</p>
        <a class="mail-link" href="mailto:office@artbackstage.net?subject=FAQ Question">
  ...
  window.castVote = function (id, direction) {
    ...
    .then(function (data) {
      if (data.ok) {
        ...
        if (fb) fb.textContent = 'Thank you for your feedback!';
      }
    })
    .catch(function () {
      if (btnPos) btnPos.disabled = false;
      if (btnNeg) btnNeg.disabled = false;
      if (fb) fb.textContent = 'Something went wrong. Please try again.';
    });
  };
}());
</script>
